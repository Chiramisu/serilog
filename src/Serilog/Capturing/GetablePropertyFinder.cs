...
#else
            var currentTypeInfo = type.GetType();

            while (currentTypeInfo != typeof(object))
            {
                var unseenProperties = currentTypeInfo.GetProperties().Where(p => p.CanRead &&
...
#else
                currentTypeInfo = baseType.GetType();
#endif
            }
        }
    }
